...
const DELETE_CUSTOMER_APT_URL = (customerNumber: number | string) => `${CUSTOMER_MODULE_URL}/${customerNumber}`;
...
export const getNurseRequest = async (userId: string, accessToken: string) => {
    const responseBody = axios.get(GET_NURSE_API_URL(userId), bearerAuthorization(accessToken))
...
// function: delelte customer 요청 함수 //
export const deleteCustomerRequest = async (customerNumber: number | string, accessToken: string) => {
    const responseBody = await axios.delete(DELETE_CUSTOMER_APT_URL(customerNumber), bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler)
    return responseBody;
}
