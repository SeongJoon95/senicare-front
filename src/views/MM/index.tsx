...
function PostBox({unShow,getToolList}: PostBoxProps) {
    ...
    // event Handler: 등록버튼 클릭 이벤트 처리 함수 //
...
function PatchBox({toolNumber, unShow, getToolList}: PathBoxProps) {
    ...
    const getToolResponse = (responseBody: GetToolResponseDto | ResponseDto | null) => {
        ...
        setName(name)
        setPurpose(purpose)
        setCount(String(count)) // count를 number 타입으로 받아왔기 때문에 형 변환 해줘야함.
    }
...
// interface: 용품 리스트 아이템 컴포넌트 Propertise //
...
export default function MM() {
    ...
    const { currentPage, totalPage, totalCount, viewList, pageList,
            setTotalList, initViewList, onPageClickHandler, onPreSectionClickHandler, onNextSectionClickHandler
    } = usePagination<Tool>();
        ...
        <div id='mm-wrapper'>
            ...
            <div className='bottom'>
                ...
                <div className='search-box'>
                    <input className='search-input' value={searchWord} placeholder='검색어를 입력해주세요' onChange={onSearchWordChangeHandler}/>
